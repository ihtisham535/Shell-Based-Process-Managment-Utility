...
If you want a simple "press 1 for X" menu for your presentation, you can use this Bash script. It clears the screen, shows a styled menu, and runs common commands.

Save as `psx_menu.sh` in the project root:
...
while true; do
  ...
  case "$choice" in
    1) ./psx -d ;;
    2) ./psx list ;;
    3) read -rp "PID: " pid && ./psx show "$pid" ;;
    4) ./psx stats ;;
    5) read -rp "PID to kill: " pid && ./psx kill "$pid" ;;
    6) echo "Ctrl+C to stop tailing"; tail -f psx_log.txt ;;
    0) clear; exit 0 ;;
...
Design tweaks you can swap quickly:
- Change the border to ASCII-only if needed: replace the box-drawing characters with `====================`.
- Add colors: prefix `echo` with `printf '\e[34m'` for blue and reset with `printf '\e[0m'`.
- Add a title splash: add `figlet "PSX"` (install with `sudo apt install figlet`) before the menu.
